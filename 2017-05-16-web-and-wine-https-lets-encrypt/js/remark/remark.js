...
function() {
  ...
  string with the demo to highlight. Returns an object with the following
  properties:
  ...
  */
  ...
  /*
  Highlighting with language detection. Accepts a string with the demo to
  ...
  */
  ...
  /*
  Applies highlighting to a DOM node containing demo. Accepts a DOM node and
  ...
  */
  ...
  /*
  Applies highlighting to all <pre><demo>..</demo></pre> blocks on a page.
  */
  function initHighlighting() {
    ...
    var blocks = document.querySelectorAll('pre demo');
...
function cleanInput(source) {
  // If all lines are indented, we should trim them all to the same point so that demo doesn't
...
function highlightCodeBlocks (content, slideshow) {
  ...
  codeBlocks.forEach(function (block) {
    if (block.parentElement.tagName !== 'PRE') {
      utils.addClass(block, 'remark-inline-demo');
      return;
    }
    ...
    utils.addClass(block, 'remark-demo');
  });
}
...
function wrapLines (block) {
  var lines = block.innerHTML.split(/\r?\n/).map(function (line) {
    return '<div class="remark-demo-line">' + line + '</div>';
  });
...
function highlightBlockLines (block, lines) {
  lines.forEach(function (i) {
    utils.addClass(block.childNodes[i], 'remark-demo-line-highlighted');
  });
}

function highlightBlockSpans (block) {
  var pattern = /([^\\`])`([^`]+?)`/g
    , replacement = '$1<span class="remark-demo-span-highlighted">$2</span>'
    ;
...
block._tag = '(?!(?:'
  + 'a|em|strong|small|s|cite|q|dfn|abbr|data|time|demo'
